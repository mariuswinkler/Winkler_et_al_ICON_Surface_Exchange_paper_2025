...
def compute_exchange_coefficients_for_threshold(min_wind_threshold, mwind_values, scaling_factor=1.0, threshold_value=0.0):
    dz = 12.5
    ...
    for mwind in mwind_values:
        cD, cH, cD_neutral, cH_neutral, RIB, mwind_out, stab_mom, stab_heat = fn.sfc_exchange_coefficients_with_cH_boost(
            dz, pqm1, thetam1, mwind, rough_m, theta_sfc, qsat_sfc,
            scaling_factor, threshold_value, min_wind_threshold
        )
        arrays['cD'].append(cD)
        arrays['cH'].append(cH)
...
thresholds = {
    "no_scaling": {"wind_threshold": 1.0, "scaling_factor": 1.0, "scaling_threshold": 0.0},
    "scaling": {"wind_threshold": 1.0, "scaling_factor": 3.0, "scaling_threshold": 6.0},
}

results = {
    key: compute_exchange_coefficients_for_threshold(
        cfg["wind_threshold"], mwind_values,
        cfg["scaling_factor"], cfg["scaling_threshold"]
    )
...
fig.suptitle(f'Forward:\n Scaling ICON "sfc_exchange_coefficients" Routine', y=0.94)
...
ax1.plot(mwind_values, results['no_scaling']['mwind_out'], color='red', label='no_scaling')
ax1.plot(mwind_values, results['scaling']['mwind_out'], color='blue', label='scaling')
...
ax2.plot(mwind_values, results['no_scaling']['RIB'], color='red', label='no_scaling')
ax2.plot(mwind_values, results['scaling']['RIB'], color='blue', label='scaling')
...
ax3.plot(mwind_values, results['no_scaling']['stab_mom'], color='red', ls='solid', label='no_scaling')
ax3.plot(mwind_values, results['scaling']['stab_mom'], color='blue', ls='solid', label='scaling')
...
ax4.plot(mwind_values, results['no_scaling']['cD'], color='red', label='no_scaling')
ax4.plot(mwind_values, results['scaling']['cD'], color='blue', label='scaling')
...
ax5.plot(mwind_values, results['no_scaling']['pr'], color='red', label='no_scaling')
ax5.plot(mwind_values, results['scaling']['pr'], color='blue', label='scaling')
...
ax6.plot(mwind_values, results['no_scaling']['km'], color='red', label='no_scaling')
ax6.plot(mwind_values, results['scaling']['km'], color='blue', label='scaling')
...
ax7.plot(mwind_values, results['no_scaling']['tau'], color='red', label='no_scaling')
ax7.plot(mwind_values, results['scaling']['tau'], color='blue', label='scaling')
...
ax8.plot(mwind_values, results['no_scaling']['tau'], color='red', label='no_scaling')
ax8.plot(mwind_values, results['scaling']['tau'], color='blue', label='scaling')
...
filename = f'01.0_components_of_subroutine_scaling_vs_no_scaling.png'
...
#%%
cD_diag_no_scaling = (results['no_scaling']['tau'])**(0.5) / mwind_values #mwind_out_array
cD_diag_scaling = (results['scaling']['tau'])**(0.5) / mwind_values #mwind_out_array_scaling
fig = plt.figure(figsize=(15, 10), facecolor="w", edgecolor="k")
fig.suptitle(f'Forward:\n Scaling ICON "sfc_exchange_coefficients" Routine', y=1)
...
ax.plot(mwind_values, results['no_scaling']['cD'], color='red', label='no_scaling')
ax.plot(mwind_values, results['scaling']['cD'], color='blue', label='scaling')
...
ax.plot(mwind_values, results['no_scaling']['tau'], color='red', label='no_scaling')
ax.plot(mwind_values, results['scaling']['tau'], color='blue', label='scaling')
#ax.plot(mwind_values, tau, color='lightsteelblue', ls='dashed', label='no_scaling & scaling')
...
ax.plot(mwind_values, cD_diag_no_scaling, color='red', label='no_scaling')
ax.plot(mwind_values, cD_diag_scaling, color='blue', label='scaling')
#ax.plot(mwind_values, tau, color='lightsteelblue', ls='dashed', label='no_scaling & scaling')
...
filename = f'01.1_cD_vs_tau_scaling_vs_no_scaling.png'
plt.savefig('figs/'+filename, facecolor='white', bbox_inches='tight')
...
fig.suptitle(f'Forward:\n Scaling ICON "sfc_exchange_coefficients" Routine', y=1.1)

G = gridspec.GridSpec(1, 2, hspace=0.5, wspace=0.3)
...
ax.plot(mwind_values, results['no_scaling']['cH'], color='red', label='no_scaling')
ax.plot(mwind_values, results['scaling']['cH'], color='blue', label='scaling')
#ax.plot(mwind_values, tau, color='lightsteelblue', ls='dashed', label='no_scaling & scaling')

ax.set_title(r"$c_H$ from Subroutine", fontsize=SIZE)
ax.set_ylabel(r"/ $-$ ")
ax.set_xlabel(r"Input Wind Speed $V_{10}$ / ms$^{-1}$")
ax.set_xlim(left=0, right=10)
ax.set_xticks([0,5,10])
...
ax.legend(loc="upper center", bbox_to_anchor=(-0.15, -0.23), fancybox=True, shadow=False, ncol=2, fontsize=SIZE)

#plt.tight_layout()

filename = f'01.2_cH_vs_tau_scaling_vs_no_scaling.png'
plt.savefig('figs/'+filename, facecolor='white', bbox_inches='tight')
